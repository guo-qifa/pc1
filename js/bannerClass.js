function Banner(id, url, interval) {
    ...
    this.url = url;
    this.interval = interval || 4000; //如果第三个参数不传，那么就默认为2000
    this.timer = null;
    this.step = 0;
    this.data = null;
...
Banner.prototype = {
    ...
    bindData: function () {
        if (this.data) {
            ...
            for (var i = 0; i < this.data.length; i++) {
                ...
                str += '<div><img src="" realSrc="' + curData.c + '"/></div>';
                strLi +='<li><img src="' + curData.sSrc + '" alt=""></li>';
            }
    ...
    setBannerImg : function () {
        ...
        for (var i = 0; i < this.lis.length; i++) { //焦点对齐
            this.lis[i].style.borderColor = this.step == i ? '#ff9d00' : '';
        }
    },
    mouseEvent : function (){
        var that = this;
